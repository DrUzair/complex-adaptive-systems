...
to setup-population
  (foreach [0.25 0.25 0.25 0.25] ["child" "teen" "middle-age" "old"] [0.5 0.6 1 0.8]
   ;[ [a b] -> show word "the sum is: " (word a  b) ]
...
to go
  ...
  [
    ;show sort-on [(- nb-infected-total)] turtles
    stop
  ]
...
"Never Infected" 1.0 0 -7500403 true "" ";plot ( ( (count turtles with [ not is_infected? and not is_cured?]) / total-population) * 100)\nplot( ( (total-population - count turtles with [is_infected? or is_cured?] ) / total-population) * 100)"
...
"middle" 1.0 0 -6459832 true "" "plot count turtles with [age-group = \"middle-age\" and is_infected?]"
...
"middle-age" 1.0 0 -6459832 true "" "plot count turtles with [not is_alive? and age-group = \"middle-age\"]"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(count turtles with [age-group = "child" and is_infected?])</metric>
    <metric>count turtles with [age-group = "teen" and is_infected?]</metric>
    <metric>count turtles with [age-group = "middle-age" and is_infected?]</metric>
    <metric>count turtles with [age-group = "old" and is_infected?]</metric>
    <metric>sum [ nb-infected ] of turtles</metric>
    <metric>sum [ nb-infected ] of turtles</metric>
